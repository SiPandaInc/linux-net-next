...
/* Kernel API kref_put() must need a non-NULL callback, since we don't need to do anything during
...
static inline bool kparser_link_attach(const void *owner_obj,
				       ...
				       const char *op)
{
	struct kparser_obj_link_ctx *reflist = kzalloc(sizeof(*reflist),
			GFP_KERNEL);
...
/* Generic kParser KMOD's netlink msg handler's definitions */
...
/* It allocates autogen IDs for hash keys if user did not allocate themselves
 * TODO: free ids
 */
...
static inline bool kparser_allocate_key_id(enum kparser_global_namespace_ids ns_id,
...
static inline bool kparser_conf_key_manager(enum kparser_global_namespace_ids ns_id,
					    const struct kparser_hkey *key,
					    struct kparser_hkey *new_key,
					    struct kparser_cmd_rsp_hdr *rsp,
					    const char *op)
...
static inline int kparser_namespace_remove(enum kparser_global_namespace_ids ns_id,
...
static inline int kparser_namespace_insert(enum kparser_global_namespace_ids ns_id,
					   struct rhash_head *obj_id,
					   struct rhash_head *obj_name)
{
	int rc;
...
static inline int alloc_first_rsp(struct kparser_cmd_rsp_hdr **rsp, size_t *rsp_len, int nsid)
{
			       ...
			       size_t glueoffset)
{
	...
	*kobj = kzalloc(kobjsize, GFP_KERNEL);
	...
	(void)snprintf(rsp->err_str_buf, sizeof(rsp->err_str_buf), "%s:Success", op);
	rsp->key = *newkey;
			     ...
			     size_t *rsp_len, __u8 recursive_read, const char *op)
{
	...
	(void)snprintf((*rsp)->err_str_buf, sizeof((*rsp)->err_str_buf), "Operation successful");
done:
	mutex_unlock(&kparser_config_lock);
			   ...
			   size_t *rsp_len, const char *op)
{
	...
	if (!kparser_conf_key_manager(conf->namespace_id, &arg->key, &key, *rsp, op)) {
		pr_alert("here:%s:%d\n", __func__, __LINE__);
		goto done;
	}
				 ...
				 size_t *rsp_len, const char *op)
{
	...
	if (kparser_namespace_lookup(conf->namespace_id, &key)) {
		(*rsp)->op_ret_code = EEXIST;
		(void)snprintf((*rsp)->err_str_buf,
				sizeof((*rsp)->err_str_buf), "%s: Duplicate object key", __func__);
		goto done;
	}
	...
	if (!table) {
		(*rsp)->op_ret_code = ENOMEM;
		(void)snprintf((*rsp)->err_str_buf,
				sizeof((*rsp)->err_str_buf), "%s: kzalloc() failed", __func__);
		goto done;
	}
			 ...
			 size_t *rsp_len, __u8 recursive_read, const char *op)
{
	...
	if (kparser_link_detach(kmdl, &kmdl->glue.owner_list, &kmdl->glue.owned_list, *rsp) != true)
		goto done;
			   ...
			   size_t *rsp_len, __u8 recursive_read, const char *op)
{
	...
	if (kparser_link_detach(kparsenode, &kparsenode->glue.glue.owner_list,
				&kparsenode->glue.glue.owned_list, *rsp) != true)
		goto done;
	...
	*proto_table = kparser_namespace_lookup(KPARSER_NS_FLAG_FIELD_TABLE, &arg->key);
	...
	if (!kflagent) {
		...
		(void)snprintf(rsp->err_str_buf,
				...
				"%s: Object key:{%s:%u} not found",
				__func__,
				arg->elem_key.name, arg->elem_key.id);
		return false;
	}
		       ...
		       size_t *rsp_len, __u8 recursive_read, const char *op)
{
	...
	if (kparser_link_detach(kparser, &kparser->glue.owner_list,
				&kparser->glue.owned_list, *rsp) != true)
		goto done;
