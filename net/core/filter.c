...
BPF_CALL_4(bpf_xdp_kparser_test, struct xdp_buff *, xdp, u32, flowd_sel,
           void *, buf, u32, len)
{
	int ret;

	ret = kparser_handler_test(xdp, flowd_sel, buf, len);
        return ret;
}
                      ...
                      void *_metadata, size_t metadata_len)
{
        ...
        if (!kparser_funchooks.__kparser_parse_hook)
        {
                ...
                start_time = ktime_get();
#endif
                rc = kparser_funchooks.__kparser_parse_hook(parser, data, pktlen, _metadata, metadata_len);
#if DEBUG
                stop_time = ktime_get();
                elapsed_time= ktime_sub(stop_time, start_time);
