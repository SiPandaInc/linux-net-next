...
ipcmd parser create metadata-rule name ipv4_node.md.hdr type hdrdata hdr-src-off 8 md-off 6  length 4
ipcmd parser create metadata-rule name ipv4_node.md.nhdr type nibbs_hdrdata hdr-src-off 10 md-off 8  length 4
ipcmd parser create metadata-rule name ipv4_node.md.hdrlen type hdrlen hdr-src-off 9 md-off 10 length 4 
ipcmd parser create metadata-rule name ipv4_node.md.cbyte type constant_byte constantvalue 89 md-off 12 length 4 
ipcmd parser create metadata-rule name ipv4_node.md.hwcbyte type constant_halfword constantvalue 145 md-off 14  length 4

ipcmd parser create metadata-rule name md.boffset type bit_offset addoff 30  md-off 16 length 4
ipcmd parser create metadata-rule name md.offset type offset addoff 55  md-off 18 length 4
ipcmd parser create metadata-rule name md.tstamp type timestamp md-off 20 length 4
...
ipcmd parser create metadata-rule name md.num_nodes type numnodes md-off 0 length 4 
ipcmd parser create metadata-rule name md.num_encaps type numencaps md-off 2  length 4
ipcmd parser create metadata-rule name md.return_code type return_code md-off 4  length 4
...
# Create parsers
ipcmd parser create parser name test_parser136 rootnode ether_node oknode node.exit0 failnode node.exit256  metametasize 44
