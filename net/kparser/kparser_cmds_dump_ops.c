					  ...
					  *pfnext_proto)
{
	...
	KPARSER_KMOD_DEBUG_PRINT(KPARSER_F_DEBUG_CLI, "pfnext_proto.src_off:%u\n", pfnext_proto->src_off);
	KPARSER_KMOD_DEBUG_PRINT(KPARSER_F_DEBUG_CLI, "pfnext_proto.mask:%u\n", pfnext_proto->mask);
	KPARSER_KMOD_DEBUG_PRINT(KPARSER_F_DEBUG_CLI, "pfnext_proto.size:%u\n", pfnext_proto->size);
	KPARSER_KMOD_DEBUG_PRINT(KPARSER_F_DEBUG_CLI, "pfnext_proto.right_shift:%u\n", pfnext_proto->right_shift);
...
static void kparser_dump_cond_expr(const struct kparser_condexpr_expr *obj)
{
	...
	KPARSER_KMOD_DEBUG_PRINT(KPARSER_F_DEBUG_CLI, "type:%u, src_off:%u, len:%u, mask:%04x value:%04x\n",
...
static void kparser_dump_cond_table(const struct kparser_condexpr_table *obj)
{
	...
	KPARSER_KMOD_DEBUG_PRINT(KPARSER_F_DEBUG_CLI, "default_fail:%d, type:%u\n", obj->default_fail, obj->type);
	KPARSER_KMOD_DEBUG_PRINT(KPARSER_F_DEBUG_CLI, "num_ents:%u, entries:%p\n", obj->num_ents, obj->entries);
...
static void kparser_dump_proto_node(const struct kparser_proto_node *obj)
{
	...
	KPARSER_KMOD_DEBUG_PRINT(KPARSER_F_DEBUG_CLI, "ops.flag_fields_length:%d\n", obj->ops.flag_fields_length);

	KPARSER_KMOD_DEBUG_PRINT(KPARSER_F_DEBUG_CLI, "ops.len_parameterized:%d\n", obj->ops.len_parameterized);
	kparser_dump_param_len(&obj->ops.pflen);

	kparser_dump_param_next_proto(&obj->ops.pfnext_proto);

	KPARSER_KMOD_DEBUG_PRINT(KPARSER_F_DEBUG_CLI, "ops.cond_exprs_parameterized:%d\n", obj->ops.cond_exprs_parameterized);
...
static void kparser_dump_proto_tlvs_table(const struct kparser_proto_tlvs_table *obj)
{
	...
	for (i = 0; i < obj->num_ents; i++) {
		KPARSER_KMOD_DEBUG_PRINT(KPARSER_F_DEBUG_CLI, "[%d]: val: %04x\n", i, obj->entries[i].type);
...
static void kparser_dump_tlv_parse_node(const struct kparser_parse_tlv_node *obj)
{
	...
	KPARSER_KMOD_DEBUG_PRINT(KPARSER_F_DEBUG_CLI, "unknown_tlv_type_ret:%d\n", obj->unknown_overlay_ret);

	KPARSER_KMOD_DEBUG_PRINT(KPARSER_F_DEBUG_CLI, "proto_tlv_node.min_len: %lu\n", obj->proto_tlv_node.min_len);
	KPARSER_KMOD_DEBUG_PRINT(KPARSER_F_DEBUG_CLI, "proto_tlv_node.max_len: %lu\n", obj->proto_tlv_node.max_len);
	KPARSER_KMOD_DEBUG_PRINT(KPARSER_F_DEBUG_CLI, "proto_tlv_node.is_padding: %u\n", obj->proto_tlv_node.is_padding);
	KPARSER_KMOD_DEBUG_PRINT(KPARSER_F_DEBUG_CLI, "proto_tlv_node.overlay_type_parameterized: %u\n",
		 obj->proto_tlv_node.ops.overlay_type_parameterized);
	kparser_dump_param_next_proto(&obj->proto_tlv_node.ops.pfoverlay_type);
	KPARSER_KMOD_DEBUG_PRINT(KPARSER_F_DEBUG_CLI, "proto_tlv_node.cond_exprs_parameterized: %u\n",
...
static void kparser_dump_tlvs_parse_node(const struct kparser_parse_tlvs_node *obj)
{
	...
	KPARSER_KMOD_DEBUG_PRINT(KPARSER_F_DEBUG_CLI, "config:max_loop: %u\n", obj->config.max_loop);
	KPARSER_KMOD_DEBUG_PRINT(KPARSER_F_DEBUG_CLI, "config:max_non: %u\n", obj->config.max_non);
	KPARSER_KMOD_DEBUG_PRINT(KPARSER_F_DEBUG_CLI, "config:max_plen: %u\n", obj->config.max_plen);
	KPARSER_KMOD_DEBUG_PRINT(KPARSER_F_DEBUG_CLI, "config:max_c_pad: %u\n", obj->config.max_c_pad);
	KPARSER_KMOD_DEBUG_PRINT(KPARSER_F_DEBUG_CLI, "config:disp_limit_exceed: %u\n", obj->config.disp_limit_exceed);
	KPARSER_KMOD_DEBUG_PRINT(KPARSER_F_DEBUG_CLI, "config:exceed_loop_cnt_is_err: %u\n", obj->config.exceed_loop_cnt_is_err);
...
static void kparser_dump_tlvs_proto_node(const struct kparser_proto_tlvs_node *obj)
{
	...
	KPARSER_KMOD_DEBUG_PRINT(KPARSER_F_DEBUG_CLI, "start_offset:%lu\n", obj->start_offset);
	KPARSER_KMOD_DEBUG_PRINT(KPARSER_F_DEBUG_CLI, "pad1_val:%u\n", obj->pad1_val);
	...
	KPARSER_KMOD_DEBUG_PRINT(KPARSER_F_DEBUG_CLI, "fixed_start_offset:%u\n", obj->fixed_start_offset);
	KPARSER_KMOD_DEBUG_PRINT(KPARSER_F_DEBUG_CLI, "min_len:%lu\n", obj->min_len);
...
static void kparser_dump_flag_field(const struct kparser_flag_field *obj)
{
	...
	KPARSER_KMOD_DEBUG_PRINT(KPARSER_F_DEBUG_CLI, "flag:%04x, mask:%04x size:%lu\n", obj->flag, obj->mask, obj->size);
...
static void kparser_dump_flag_fields(const struct kparser_flag_fields *obj)
{
	...
	KPARSER_KMOD_DEBUG_PRINT(KPARSER_F_DEBUG_CLI, "num_idx:%lu, fields:%p\n", obj->num_idx, obj->fields);
...
static void kparser_dump_proto_flag_fields_table(const struct kparser_proto_flag_fields_table *obj)
{
	...
	KPARSER_KMOD_DEBUG_PRINT(KPARSER_F_DEBUG_CLI, "num_ents:%d, entries:%p\n", obj->num_ents, obj->entries);

	if (!obj->entries)
		goto done;

	for (i = 0; i < obj->num_ents; i++) {
		KPARSER_KMOD_DEBUG_PRINT(KPARSER_F_DEBUG_CLI, "proto_flag_fields_table_entry_flag:%x\n", obj->entries[i].flag);
...
static void kparser_dump_flags_proto_node(const struct kparser_proto_flag_fields_node *obj)
{
	...
	KPARSER_KMOD_DEBUG_PRINT(KPARSER_F_DEBUG_CLI, "ops.get_flags_parameterized:%d\n", obj->ops.get_flags_parameterized);
	KPARSER_KMOD_DEBUG_PRINT(KPARSER_F_DEBUG_CLI, "ops.pfget_flags: src_off:%u mask:%04x size:%u\n",
...
static void kparser_dump_metadatatable(const struct kparser_metadata_table *obj)
{
	...
	for (i = 0; i < obj->num_ents; i++)
		KPARSER_KMOD_DEBUG_PRINT(KPARSER_F_DEBUG_CLI, "mde[%d]:%04x\n", i, obj->entries[i].val);
...
static void kparser_dump_proto_table(const struct kparser_proto_table *obj)
{
	...
	for (i = 0; i < obj->num_ents; i++) {
		KPARSER_KMOD_DEBUG_PRINT(KPARSER_F_DEBUG_CLI, "[%d]: val: %d\n", i, obj->entries[i].value);
...
static void kparser_dump_node(const struct kparser_parse_node *obj)
{
	...
	KPARSER_KMOD_DEBUG_PRINT(KPARSER_F_DEBUG_CLI, "name: %s: type: %d\n", obj->name, obj->node_type);
	...
	default:
		KPARSER_KMOD_DEBUG_PRINT(KPARSER_F_DEBUG_CLI, "unknown node type:%d\n", obj->node_type);
		break;
	}
...
void kparser_dump_parser_tree(const struct kparser_parser *obj)
{
	...
	KPARSER_KMOD_DEBUG_PRINT(KPARSER_F_DEBUG_CLI, "config: max_nodes:%u\n", obj->config.max_nodes);
	KPARSER_KMOD_DEBUG_PRINT(KPARSER_F_DEBUG_CLI, "config: max_encaps:%u\n", obj->config.max_encaps);
	KPARSER_KMOD_DEBUG_PRINT(KPARSER_F_DEBUG_CLI, "config: max_frames:%u\n", obj->config.max_frames);
	KPARSER_KMOD_DEBUG_PRINT(KPARSER_F_DEBUG_CLI, "config: metameta_size:%lu\n", obj->config.metameta_size);
	KPARSER_KMOD_DEBUG_PRINT(KPARSER_F_DEBUG_CLI, "config: frame_size:%lu\n", obj->config.frame_size);
	...
	for (i = 0; i < (sizeof(obj->cntrs_conf.cntrs) /
		...
		if (obj->cntrs)
			KPARSER_KMOD_DEBUG_PRINT(KPARSER_F_DEBUG_CLI, "cntr[%d]:%d", i, obj->cntrs->cntr[i]);
	}
